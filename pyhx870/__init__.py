...
class HX870(object):
    ...
    def sync(self, flush_output=False, flush_input=True):
        ...
        if r.type != "#CMDOK":
            logger.warning("Radio failed to sync, trying harder")
            self.conn.flush_output()
            time.sleep(0.1)
            self.conn.flush_input()
            self.write(Message("#CMDSY"))
            r = self.receive()  # expect #CMDOK
            if r.type != "#CMDOK":
                logger.error("Radio failed to sync, giving up")
    ...
    def write_mmsi(self, mmsi=None, status=None):
        if mmsi is None:
            mmsi = "FFFFFFFFFF"
            if status is None:
                status = "00"
    ...
    def write_atis(self, atis=None, status=None):
        if atis is None:
            atis = "FFFFFFFFFF"
            if status is None:
                status = "00"
            ...
            for x in atis:
                if x not in "0123456789":
                    invalid_digits = True
                    break
            if not atis[0] == "9" or invalid_digits:
